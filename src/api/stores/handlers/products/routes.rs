...
pub async fn create_new_product(db: AxumDBExtansion, JsonWithValidation(payload): JsonWithValidation<CreateProductPayload>) {
        todo!()
...
pub async fn upload_product_asset(db: AxumDBExtansion,
...
pub async fn get_product(db: AxumDBExtansion, Path(product_id): Path<ObjectId>) {
    todo!()
}
